...
plot.append('set ylabel "IO (Kilobytes)";')
plot.append('set ytics nomirror;')
plot.append('set y2range [0:103];')
plot.append('set y2label "Memory/CPU utilization (%)";')
plot.append('set y2tics 20;')
plot.append('set grid linetype 0;')
plot.append('plot ')
plot.append('"%s" using 19:xtic(19) title "t",' % vmstat[1])
plot.append('"%s" using 0:10 title "io:in" smooth bezier lt 1 lw 2,' % vmstat[1])
plot.append('"%s" using 0:9 title "io:out" smooth bezier lt 5 lw 2,' % vmstat[1])
plot.append('"%s" using 0:13 title "cpu:user" axis x1y2 smooth bezier lt 3 lw 1,' % vmstat[1])
plot.append('"%s" using 0:14 title "cpu:sys" axis x1y2 smooth bezier lt 4 lw 1,' % vmstat[1])
plot.append('"%s" using 0:15 title "cpu:idle" axis x1y2 smooth bezier lt 2 lw 2,' % vmstat[1])
plot.append('"%s" using 0:($4/%d) title "mem:free" axis x1y2 smooth bezier lt 6 lw 2,' % (vmstat[1], 10*options.ram))
plot.append('"%s" using 0:($5/%d) title "mem:buff" axis x1y2 smooth bezier lt 7 lw 1,' % (vmstat[1], 10*options.ram))
plot.append('"%s" using 0:($6/%d) title "mem:cache" axis x1y2 smooth bezier lt 8 lw 1,' % (vmstat[1], 10*options.ram))
plot.append('"%s" using 0:($3/%d) title "mem:swapped" axis x1y2 smooth bezier lt 9 lw 2' % (vmstat[1], 10*options.ram))
